...
- There are currently 3 post layouts available: `PostLayout`, `PostSimple` and `PostGallery`. `PostLayout` is the default 2 column layout with meta and author information. `PostSimple` is a simplified version of `PostLayout`, while `PostGallery` features a banner image.
